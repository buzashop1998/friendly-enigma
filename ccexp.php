            ...
            if (is_numeric($xx))
            {
                ...
                switch ($yy)
                {
                    ...
                    case 2:
                        if (($xx >= 1) and ($xx <= 12) and (!isset($ccnum['mon'])))
                            $ccnum['mon'] = $xx;
                        elseif (($xx >= 11) and ($xx <= 19) and (isset($ccnum['mon'])) and (!isset($ccnum['year'])))
                            $ccnum['year'] = "20" . $xx;
                        break;
                    case 4:
                        if (($xx >= 2011) and ($xx <= 2019) and (isset($ccnum['mon'])))
                            $ccnum['year'] = $xx;
            ...
            if ($count == 1)
            {
                ...
                if ($credit >= $fullprice)
                {
                    ...
                    foreach ($cclist as $ccline)
                    {
                        ...
                        if ($ccnum)
                        {
                            if (_bin($ccnum))
                            {
                                ...
                                if ($okokok == 1)
                                {
                                    ...
                                    echo $relog . '/' . $tongso . '.<font color="green">Live | </font><font color="blue"> ' . $_INFO . '</font><br>';
                                    $cc['l'][] = $ccline;
                                }
                                if ($okokok == 2)
                                {
                                    echo $relog . '/' . $tongso . '.<font color="red"> Die | ' . $_INFO . '</font><br>';
                                    $cc['d'][] = $ccline;
                                    $sql = "SELECT credit from " . $config["table_users"] . " WHERE userId = '" . clean($_SESSION["userId"]) . "'";
                                    $result = mysql_query($sql, $data_sql);
                                    if ($result)
                                ...
                                if ($okokok == -1)
                                {
                                    echo $relog . '/' . $tongso . '.<font color="black">Invaild | ' . $_INFO . '</font><br>';
                                    $cc['u'][] = $ccline;
                                }
                                if ($okokok == 3)
                                {
                                    echo $relog . '/' . $tongso . '.<font color=red>Unknown | ' . $_INFO . '</font><br>';
                                    $cc['u'][] = $ccline;
                                }
                                if ($okokok == 5)
                                {
                                    echo $relog . '/' . $tongso . '.<font color="yellow">Live[<font color="green">G</font>] | ' . $_INFO . '</font><br>';
                                    $cc['l'][] = $ccline;
                                }
                                if ($okokok == 6)
                                {
                                    echo $relog . '/' . $tongso . '.<font color="red">Exdate/CCV2 | ' . $_INFO . '</font><br>';
                                    $cc['u'][] = $ccline;
                                }
                        ...
                        {
                            echo $relog . '/' . $tongso . '.<font color=black>Line_Error | ' . substr($ccline, 0, 80) . ' [...]</font><br>';
                            $cc['e'][] = $ccline;
                        }
                        flush();
                    }
